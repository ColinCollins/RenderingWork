...
prop.renderSquare = function (x, y) {
    // triangle1
    let imgWidth = this.image.width;
    let imgHeight = this.image.height;
    // 设定好与原 texture 相同尺寸
    let pos1 = new Vec3((imgWidth / 2 + x), (imgHeight / 2 + y), 0);
    let pos2 = new Vec3((-imgWidth / 2 + x), (imgHeight / 2 + y), 0);
    let pos3 = new Vec3((-imgWidth / 2 + x), (-imgHeight / 2 + y), 0);
    let pos4 = new Vec3((imgWidth / 2 + x), (-imgHeight / 2 + y), 0);

    let point1 = new Point(pos1, null, new Vec3(1, 1, 0));
    let point2 = new Point(pos2, null, new Vec3(-1, 1, 0));
    let point3 = new Point(pos3, null, new Vec3(-1, -1, 0));
    let point4 = new Point(pos4, null, new Vec3(1, -1, 0));
